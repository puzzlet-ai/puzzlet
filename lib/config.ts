...
import { extractOverrideSettings, omit, deepClone, generateUniqueId } from "./utils";
...
export class AIConfigRuntime implements AIConfig {
   ...
   */
  public toString(saveOptions?: SaveOptions) {
  ...
  public addPrompt(prompt: Prompt, promptName?: string) {
    const name = promptName || prompt.name || `prompt_${generateUniqueId()}`;
  ...
  public getMetadata(promptName?: string) {
    if (promptName) {
      ...
      const globalMetadataWithoutModels = omit(globalMetadata, "models");
